# Website's address

https://daei-fastfood-ali.ir

# Intro
وبسایت منوی دیجیتال رستوران دایی علی کرمان، شهرک باهنر. اولین کار فرانت اند real world من علاوه بر طراحی UI و Deployment. 

# Description
پروژه mobile responsive برای منوی دیجیتال و اسکن شده کیو آر کد. وبسایت استاتیک single page application به دلیل اندازه و سایز وبسایت و نیاز اساسی به حداقل load time و حداکثر performance. 

# Technologies
وبسایت از ساختار ساده SPA استفاده میکنه. یک index.html به عنوان شل برای صفحات HTML کامپوننت هستند که به صورت دینامیک به همراه عکس ها توسط جاوا اسکریپت لود میشن. 
عکس های header به فرمت webp تبدیل شده و در سرور هاست cache میشن. 


